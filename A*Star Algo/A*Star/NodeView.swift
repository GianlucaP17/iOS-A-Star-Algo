...
let INFINITE = 99999 // make dummy g & h values

class NodeView: UIView { // view that hold all nodes
    
    let size: Int = 10 // size of the grid 8x8
    var nodes: [[Node]] = [] // all the nodes, in b dimensional node array
    
    override func awakeFromNib() {
        for _ in 0..<size { // init nodes with just air
            var final: [Node] = []
            for _ in 0..<size {
                let node = Node()
                node.type = .Air
        ...
        nodes[size-1][0].type = .pointA //creo point a
        nodes[0][size-1].type = .pointB //creo point b
        self.setNeedsDisplay() // refresh view
        for i in 0..<nodes.count { // create x & y vls for each node (in arr)
    ...
    //label inside blocks with all its info, position, size etc...
    func createLabel(text: String, pos: CGPoint, size_: CGSize) {
        ...
        label.text = text
        //self.addSubview(label) // uncomment to add lael with calculations
    }
    ...
    //customize blocks touched by users
    ...
    // reconstruct oath that the algo took
    ...
    //thw algo itself (from book text to real code)
